...
for (let i=10; i>=0; --i){
    if (i%2!==0) continue
    else if(i!==0) {evenNumbersResult+=`${i}-`}
    else {evenNumbersResult+=i}
}
...
for (let i = 0; i<(text.length); i++ ){
    if (text[i]!==' ') replaceSpacesWithOneResult+=text[i]
    else if (text[i]===' ') replaceSpacesWithOneResult+='1'
}
